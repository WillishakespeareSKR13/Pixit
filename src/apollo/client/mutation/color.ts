...
export const GENERATEPRODUCSCOLORS = gql`
  mutation generateProductsColors($input: InputGenerateProductsColors!) {
    generateProductsColors(input: $input) {
      id
    }
  }
`;

export const UPDATEPRODUCTSCOLORS = gql`
  mutation updateProductsColors($id: ID!) {
    updateProductsColors(id: $id) {
      id
      name
      price
    }
