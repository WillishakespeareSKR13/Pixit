...
import DownloadPdf from '@Src/components/@atoms/AtomPdfDashboard';
...
import AtomLoading from '@Src/components/@atoms/AtomLoading';
...
const OrganismsConvertImage: FC = () => {
  ...
  const [selectedRoom, setSelectedRoom] = useState('REAL');
  ...
  useEffect(() => {
    if (cropImagesBlock.length === quantity) {
      setLoading(false);
      setTimeout(() => {
            ...
            >
              {loading || isLoading ? (
                <AtomLoader
                  ...
                  type="small"
                  customCSS={css`
                    width: ${90 * (selectedConfig?.x ?? 0)}px;
                    height: ${90 * (selectedConfig?.y ?? 0)}px;
                    .lds-ring {
                      width: 30px;
                      height: 30px;
                      div {
                ...
                >
                  {cropImagesBlock.length > 0 ? (
                    ...
                    >
                      <AtomText
                        color="#4a4a54"
                        fontSize={`${(90 * (selectedConfig?.x ?? 0)) / 6}px`}
                        fontWeight={600}
                        customCSS={css`
                          text-align: center;
                        `}
                      ...
                      >
                        <StyledImage
                          ...
                          customCSS={css`
                            ...
                            height: ${((selectedRoomSizeConfig?.height ?? 0) *
                              3 ?? 0) / (selectedConfig?.y ?? 0)}px;
                          `}
                        />
                      </AtomButton>
                    ))}
                ...
                >
                  {cropImagesBlock.length > 0 ? (
                    ...
                    >
                      <AtomText
                        color="#4a4a54"
                        fontSize={`${(30 * (selectedConfig?.x ?? 0)) / 6}px`}
                        fontWeight={600}
                        customCSS={css`
                          text-align: center;
                        `}
  ...
  return (
    <>
      <AtomLoading isLoading={loadingLoader} />
      <AtomWrapper
        customCSS={css`
          border-radius: 10px;
          width: max-content;
